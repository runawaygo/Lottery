var	OAuth2 = require('oauth').OAuth2,
	...
	CONSUMER_KEY = '567961804',
	CONSUMER_SECRET = '6297acdb8745fe205ad0e2fa7bc1c774';	
	

var sinaOAuth2 = module.exports = function(access_key, access_secret) {
...
sinaOAuth2.prototype = {
	...
	getOAuthAccessToken:function(req,res,callback){
		...
		this.oa.getOAuthAccessToken(req.query.code,params,function(error,access_token,refresh_token){
	...
	comeon:function(args,callback){
		...
		if (!this.access_key || !this.access_secret) return callback("not authorize");
		this.oa.post(url, this.access_key, this.access_secret, args, null, function(error, data, res) {
			//data = JSON.parse(data);
			callback(error, data, res);
		});
	}
...
'unread', 'reset_count', 'friends', 'followers'].forEach(function(fnName) {
	sinaOAuth2.prototype[fnName] = function(args, callback) {
