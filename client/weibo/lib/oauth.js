...
if(typeof require !== 'undefined') {
	...
	var b64_hmac_sha1 = weibo.sha1.b64_hmac_sha1;
}
...
OAuth.SignatureMethod.registerMethodClass(["HMAC-SHA1", "HMAC-SHA1-Accessor"],
    OAuth.SignatureMethod.makeSubclass(
        function getSignature(baseString) {
        	...
        	if(crypto) {
        		// nodejs
        		signature = (new crypto.Hmac).init("sha1",this.key).update(baseString).digest("base64");
        	} else {
        		// browser client, use sha1.js
                signature = b64_hmac_sha1(this.key, baseString);
        	}
            return signature;
        }
    ));
